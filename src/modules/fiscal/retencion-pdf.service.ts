  ...
  async generatePdfBuffer(
    ...
    const row = await this.prisma.retencionIVA.findFirst({
      ...
      include: {
        expense: true,
        organization: {
          select: { nombre: true, taxId: true, legalName: true },
        },
      },
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        .fontSize(16)
        .text("Comprobante de Retencion de IVA", { align: "center" });
      doc.moveDown();
      doc.fontSize(10);
      ...
      doc.text(
        `Alicuota retencion: ${(Number(row.withholdingRate) * 100).toFixed(0)}%`,
      );
      doc
        .fontSize(12)
        .text(`Monto retenido: ${Number(row.withholdingAmount).toFixed(2)}`, {
          underline: true,
        });
      doc.moveDown(2);
      doc
        .fontSize(8)
        .text(
          "Documento generado por MARFYL — validez segun normativa SENIAT vigente.",
          {
            align: "center",
          },
        );
